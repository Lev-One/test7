document.addEventListener('DOMContentLoaded', function () {
  ...
  const rcwUrlGet = "https://citynet.get/b2b/rcw/get_schedule.php";
  const rcwUrlSet = "https://citynet.get/b2b/rcw/request_call.php";

  let rcwDatesInit = [];
  let rcwDates = [];
  let rcwCallMode = 0;
  let rcwTimeInSecondsTotal = 30000;
            ...
            <div class="rcw-modal-body" id="rcw-modal-body">
              <div id="rcw-modal-body-call">
                <div style="text-align: center;">
                  <p class="rcw-body-title">გნებავთ ჩვენ გადმოგირეკავთ?</p>
                  <p class="rcw-body-title-sub">მიუთითეთ თქვენი ნომერი და ჩვენ დაგიკავშირდებით 30 წამში!</p>
                </div>
                <div>
                  <div>
                    <div class="rcw-row">
  ...
  const rcwUnputField = document.getElementById('rcw-input');
  ...
  keypad.addEventListener('click', function (event) {
    ...
    if (clickedElement.classList.contains('rcw-key')) {
      const inputValue = rcwUnputField.value;
      const clickedValue = clickedElement.innerText;

      if (clickedValue === '←') {
        rcwUnputField.value = inputValue.slice(0, -1);
      } else if (clickedValue === 'x') {
        rcwUnputField.value = '';
      } else {
        rcwUnputField.value = inputValue + clickedValue;
      }
  ...
  rcwUnputField.addEventListener('input', function () {
    validateRcwPhoneNumber();
  });
  ...
  const validateRcwPhoneNumber = () => {
    ...
    if (phoneNumberRegex.test(rcwUnputField.value)) {
  ...
  const sendRcwData = async () => {
    ...
    if (rcwCallMode == 0) {
      time = rcwGetFormattedDateTime(new Date(), 30);
    } else {
    ...
    formData.append("apikey", rcwApiKey);
    formData.append("phone", rcwUnputField.value);
